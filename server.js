...
const RED = 'C00000';   // kept for cover page "ИНФОРМАЦИЯ" only
const BLUE = '1F4E79';
    ...
    if (match[1] !== undefined) {
      ...
      if (/^\d/.test(inner)) {
        ...
        runs.push(...inner.split(/(\d+)/).filter(t=>t).map(t =>
          /^\d+$/.test(t) ? r(t, { italic: true, color: BLUE }) : r(t, { italic: true })
        ));
        runs.push(r(')', { italic: true }));
      } else {
        // (geo) - italic blue
...
function parseMoneyStr(str) {
  ...
  while ((m = combined.exec(str)) !== null) {
    ...
    else if (m[2]) runs.push(r(m[2], { color: BLUE, bold: true })); // digits - blue bold
...
- education: хронологически от старого к новому, минимум 1, максимально полно - не пропускать ни одну ступень
- career: хронологически от старого к новому, ВСЕ должности которые найдёшь без исключения, текущая - ПОСЛЕДНЯЯ
